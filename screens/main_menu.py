...
try:
    ...
    print("ℹ️ Gesture control will be disabled. Using mouse only.")
    mp = None
    MP_AVAILABLE = False
except Exception as e:
    print(f"⚠️ Error loading MediaPipe: {e}")
    mp = None
    MP_AVAILABLE = False
...
class MainMenu:
    ...
    def __init__(self, screen):
        ...
        self.gesture_enabled = MP_AVAILABLE
        ...
        if self.gesture_enabled:
            ...
            self.cap = None
            self.camera_size = (160, 120)
            print("ℹ️ Gesture control disabled. Use mouse to interact.")

        # Gesture state (only used if gesture_enabled)
        ...
        if self.gesture_enabled:
            ...
            print(f"🖱️ Use mouse to interact with the game")
    ...
    def update_gesture(self):
        ...
        if not self.gesture_enabled or self.cap is None:
            return
    ...
    def trigger_click(self):
        """Handle click at cursor position (from gesture)"""
        pos = self.cursor_pos
        print(f"🖱️ Gesture click at: {pos}")
        self._handle_click(pos)

    def _handle_click(self, pos):
        """Internal method to handle clicks at a position"""
    ...
    def handle_event(self, event):
        # Handle mouse click as fallback when gesture control is disabled OR when no hand is detected
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # Left click
            # Check if we should use mouse (gesture disabled OR no hand detected OR in grace period)
            use_mouse = not self.gesture_enabled or self.current_gesture in ["NO HAND", "NO HAND (GRACE)"]

            if use_mouse:
                print(f"🖱️ Mouse click at: {event.pos}")
                self._handle_click(event.pos)
                return  # Return after handling mouse click

        # Route other events to active screens
        if self.current_screen == "menu":
            pass
        elif self.current_screen == "stage_select" and self.stage_select:
    ...
    def draw_camera_feed(self):
        if not self.gesture_enabled or self.camera_frame is None:
            return
    ...
    def draw_cursor(self):
        if self.current_gesture != "NO HAND" and self.gesture_enabled:
            if self.fist_start_time > 0:
    ...
    def draw(self):
        if self.current_screen == "menu":
            ...
            self.draw_camera_feed()
            # Draw cursor on top
            if self.gesture_enabled and self.current_gesture != "NO HAND":
                if self.fist_start_time > 0:
                    color = (255, 200, 0)
                else:
                    color = (255, 255, 255)
