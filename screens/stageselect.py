...
class StageSelect:
    def __init__(self, screen, main_menu):
        ...
        # ============================================================
        # MOUSE MOVEMENT (as fallback when no hand detected)
        # ============================================================
        ...
        self.dialogue_lines = [
            ...
            ("Old Man",
             "Deep inside the Geometry Forest, the magical Shapes have become lost. Only a true student adventurer can help them find their way."),
            ("Old Man", "Do you want to explore the Geometry Forest?"),
            ("Student", "Yes! I'll help!"),
            ("Old Man",
             "Excellent! Along the way, you must answer my questions about shapes and angles. If you answer correctly, the forest will guide you safely to the next path."),
        ...
        self.title_total_width = sum(l["width"] for l in self.title_letters) + self.title_spacing * (
                    len(self.title_text) - 1)
    ...
    def update_gesture(self, cursor_pos, fist_start_time, CLICK_HOLD_TIME, current_gesture):
        ...
        # If hand is detected, cancel mouse movement
    ...
    def trigger_click(self, pos):
        print(f"🖱️ StageSelect click at: {pos}")
        ...
        # Check if click is on the ground (not on UI elements) - for mouse movement
        # Get world position from screen click
        world_x = (pos[0] / ZOOM) + self.camera_x
        world_y = (pos[1] / ZOOM) + self.camera_y

        # Set mouse target for movement (only if click is on walkable tile)
        ...
        if 0 <= row < self.ROWS and 0 <= col < self.COLS:
            ...
            if tile in self.WALKABLE_TILES:
                ...
                print(f"🎯 Moving to: ({world_x}, {world_y})")
                return True

        # Trigger teleport on click when standing on a portal (Only if NPC has been spoken to)
        current_portal = None
        for portal in self.portals:
            if portal.contains_position(self.player_x, self.player_y):
                current_portal = portal
    ...
    def update(self):
            ...
            if self.npc_oldman_y < target_y:
                ...
                if self.npc_oldman_anim_timer >= 10:
                    ...
                    if self.npc_oldman_down_sprites:
                        self.npc_oldman_anim_frame = (self.npc_oldman_anim_frame + 1) % len(
                            self.npc_oldman_down_sprites)
            else:
            ...
            if self.npc_skeleton_y < target_y:
                ...
                if self.npc_skeleton_anim_timer >= 10:
                    ...
                    if self.npc_skeleton_down_sprites:
                        self.npc_skeleton_anim_frame = (self.npc_skeleton_anim_frame + 1) % len(
                            self.npc_skeleton_down_sprites)
            else:
            ...
            if self.npc_knight_y < target_y_max:
                ...
                if self.npc_knight_anim_timer >= 10:
                    ...
                    if self.npc_knight_down_sprites:
                        self.npc_knight_anim_frame = (self.npc_knight_anim_frame + 1) % len(
                            self.npc_knight_down_sprites)
        ...
        if self.teleport_cooldown <= 0:
            for portal in self.portals:
                ...
                port_rect = pygame.Rect(portal.get_world_x(), portal.get_world_y(), portal.get_width_pixels(),
                                        portal.get_height_pixels())
                if port_rect.colliderect(p_rect) or portal.contains_position(self.player_x + TILE_SIZE // 2,
                                                                             self.player_y + TILE_SIZE // 2):
    ...
    def update_player_movement(self):
        ...
        if self.oldman_dialogue_state == 1 or self.skeleton_dialogue_state == 1 or self.knight_dialogue_state == 1 or self.bromen_dialogue_state == 1 or self.player_following_target or self.player_block_timer > 0:
            # Cancel mouse movement if dialogue is active
            self.mouse_moving = False
            self.mouse_target_x = None
            self.mouse_target_y = None
            return
        ...
        # Check if hand is detected FIRST (gesture takes priority)
        if self.hand_detected and self.current_gesture not in ["NO HAND", "NO HAND (GRACE)"]:
            ...
            # Reset mouse target if hand is controlling
            self.mouse_moving = False
            self.mouse_target_x = None
            self.mouse_target_y = None

        # If no hand detected, use mouse movement
    ...
    def draw(self):
        ...
        # Draw mouse target indicator (if moving with mouse)
        if self.mouse_moving and self.mouse_target_x is not None:
            screen_x = (self.mouse_target_x - self.camera_x) * ZOOM
            screen_y = (self.mouse_target_y - self.camera_y) * ZOOM

            if 0 <= screen_x < self.width and 0 <= screen_y < self.height:
                # Draw a target indicator (pulsing circle)
                pulse = math.sin(self.frame_counter * 0.1) * 0.3 + 0.7
                alpha = int(255 * pulse)

                # Create a surface with alpha
                target_surf = pygame.Surface((32, 32), pygame.SRCALPHA)
                pygame.draw.circle(target_surf, (255, 100, 100, alpha), (16, 16), 12, 2)
                pygame.draw.circle(target_surf, (255, 255, 255, int(alpha * 0.5)), (16, 16), 4)

                self.screen.blit(target_surf, (screen_x - 16, screen_y - 16))
        ...
        # Draw locked portal banner
        if self.locked_portal_banner_timer > 0 and self.locked_portal_banner_msg:
            banner_font = pygame.font.SysFont("Comic Sans MS", 24, bold=True)
            banner_surf = banner_font.render(self.locked_portal_banner_msg, True, (255, 255, 0))
            banner_rect = banner_surf.get_rect(center=(self.width // 2, 120))
            ...
            # Draw border
            pygame.draw.rect(self.screen, (255, 215, 0), bg_rect, 3, border_radius=5)
    ...
    def draw_dialogue_box(self):
        ...
        pygame.draw.rect(dialogue_surface, (255, 215, 0, 255), (0, 0, box_width, box_height), width=3,
                         border_radius=10)  # Gold border
        ...
        if (self.frame_counter // 30) % 2 == 0:
            ...
            self.screen.blit(prompt_surface,
                             (box_x + box_width - prompt_surface.get_width() - 20, box_y + box_height - 25))
            ...
            if self.oldman_dialogue_state == 1:
                ...
                if event.key in [pygame.K_SPACE, pygame.K_RETURN]:
                    ...
                    if self.skeleton_dialogue_index >= len(self.skeleton_dialogue_lines):
                        ...
                        self.player_following_target = 'skeleton'
                        self.player_block_timer = 0
                        print("☠️ Dialog complete! Skeleton starts moving right and player follows.")
                    return "dialogue_advance"
            elif self.knight_dialogue_state == 1:
                if event.key in [pygame.K_SPACE, pygame.K_RETURN]:
                    self.knight_dialogue_index += 1
                    if self.knight_dialogue_index >= len(self.knight_dialogue_lines):
                        ...
                        self.player_following_target = 'knight'
                        self.player_block_timer = 0
                        print("⚔️ Dialog complete! Knight starts moving down and player follows.")
                    return "dialogue_advance"
            elif self.bromen_dialogue_state == 1:
                if event.key in [pygame.K_SPACE, pygame.K_RETURN]:
                    self.bromen_dialogue_index += 1
                    if self.bromen_dialogue_index >= len(self.bromen_dialogue_lines):
                        ...
                        self.player_following_target = 'bromen'
                        self.player_block_timer = 0
                        print("✨ Dialogue complete! Bromen starts moving north and player follows.")
                    return "dialogue_advance"
