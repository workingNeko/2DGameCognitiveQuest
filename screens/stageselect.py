...
class StageSelect:
    ...
    def trigger_click(self, pos):
        ...
        # Trigger teleport on click when standing on a portal
        if (self.oldman_dialogue_state in (0, 3)) and (self.skeleton_dialogue_state in (0, 3)) and (
                self.knight_dialogue_state in (0, 3)) and (self.bromen_dialogue_state in (0, 3)):
            current_portal = None
            for portal in self.portals:
                if portal.contains_position(self.player_x, self.player_y):
                    current_portal = portal
    ...
    def update_player_movement(self):
        ...
        if self.oldman_dialogue_state in (1, 2) or self.skeleton_dialogue_state in (1,
                                                                                    2) or self.knight_dialogue_state in (
                1, 2) or self.bromen_dialogue_state in (1, 2) or self.player_block_timer > 0:
    ...
    def draw(self):
        ...
        if self.npc_bromen_found:
            if self.bromen_dialogue_state == 2:
                if self.npc_bromen_teleport_sprites and self.bromen_teleport_frame < len(
                        self.npc_bromen_teleport_sprites):
