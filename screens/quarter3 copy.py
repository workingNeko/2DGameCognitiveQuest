# screens/quarter3.py - Quarter 3 Map Handler (map3.txt)
...
class Quarter3:
    def __init__(self, screen, main_menu, map_name):
        ...
        self.QUARTER3_TILES_PATH = os.path.normpath(
            r"C:\Users\Xolaani\Documents\2DGameCognitiveQuest-master\2DGameCognitiveQuest-master\assets\images\sprites\objects\tiles\quarter3tiles"
        )
        if not os.path.isdir(self.QUARTER3_TILES_PATH):
    ...
    def trigger_click(self, pos):
        ...
        # State 5: Final speech click
            ...
            if btn_rect.collidepoint(pos):
                ...
                print("🌟 All Quiz Stations Cleared! Goal Portal Active!")
    ...
    def draw_math_background(self):
        ...
        for i, color in enumerate(pastel_colors):
            ...
            cy = 40 + ((i // 5) * 80)
            pygame.draw.circle(self.screen, color, (cx, cy), 38, 0)

        for i in range(12):
            x = (i * 97) % self.width
            y = (i * 43) % (self.height // 2)
            pygame.draw.circle(self.screen, (255, 255, 255), (x, y), 2, 0)
        ...
        for label, x_pos in [("+", 70), ("=", 180), ("3", 290), ("×", 390), ("7", 520), ("-", 610)]:
            ...
            self.screen.blit(text, (x_pos, 18))
    ...
    def draw_tile(self, c, world_x, world_y):
        ...
        if (-margin <= screen_x <= self.width + margin and
            ...
            if c in ['G', '1', '2', '3', '4', '5', 'P']:
                bubble = pygame.Surface((scaled_size, scaled_size), pygame.SRCALPHA)
                pygame.draw.circle(bubble, (255, 255, 255, 70), (scaled_size // 2, scaled_size // 2), scaled_size // 3)
                self.screen.blit(bubble, (screen_x, screen_y))
